#!/usr/bin/env python3
"""
M3 Mac Optimized CREMA Training Script
Optimized for Apple M3 Air with MPS acceleration
"""
...
def main():
    parser = argparse.ArgumentParser(description="M3 Mac Optimized CREMA Training")
    ...
    parser.add_argument('--epochs', type=int, default=25)  # More epochs for better convergence
    parser.add_argument('--batch_size', type=int, default=20)  # Optimized for M3 Mac memory
    parser.add_argument('--lr', type=float, default=8e-5)  # Conservative LR for stability
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints_crema_m3')
    parser.add_argument('--resume_from', type=str, help='Path to checkpoint to resume from')
    parser.add_argument('--gradient_clip', type=float, default=0.5)  # Conservative clipping
    ...
    print(f"📊 Training on CREMA dataset with M3 Mac optimization")
    ...
    # Initialize models with M3 Mac optimizations
    ...
    classifier = AdvancedOpenMaxClassifier(
        ...
        num_layers=35, 
        base_dim=512, 
        dropout=0.2  # Moderate dropout for M3 Mac
    ).to(device)
    prototypes = PrototypeMemory(NUM_LABELS, 512).to(device)

    # M3 Mac optimized optimizer
    print("⚙️  Setting up optimizer...")
    optimizer = optim.AdamW([
        # Encoders: Very low LR, high weight decay
        {'params': audio_encoder.parameters(), 'lr': args.lr * 0.02, 'weight_decay': 0.08},
        {'params': text_encoder.parameters(), 'lr': args.lr * 0.02, 'weight_decay': 0.08},
        
        # Attention and pooling: Low LR
        {'params': cross.parameters(), 'lr': args.lr * 0.3, 'weight_decay': 0.12},
        {'params': pool_a.parameters(), 'lr': args.lr * 0.3, 'weight_decay': 0.12},
        {'params': pool_t.parameters(), 'lr': args.lr * 0.3, 'weight_decay': 0.12},
        
        # Fusion: Medium LR
        {'params': fusion.parameters(), 'lr': args.lr * 0.8, 'weight_decay': 0.1},
        
        # Classifier: Higher LR for fast adaptation
        {'params': classifier.deep_classifier.parameters(), 'lr': args.lr * 1.5, 'weight_decay': 0.15},
        {'params': classifier.anchor_clustering.parameters(), 'lr': args.lr * 2.0, 'weight_decay': 0.1},
        {'params': classifier.uncertainty_head.parameters(), 'lr': args.lr * 1.2, 'weight_decay': 0.1},
        
        # Prototypes: Medium LR
        {'params': prototypes.parameters(), 'lr': args.lr * 0.6, 'weight_decay': 0.1},
    ...
    supcon = SupConLoss(temperature=0.07)  # Optimized temperature
    ...
    print(f"🎯 M3 Mac Training Configuration:")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (audio_list, text_list, labels) in enumerate(pbar):
            ...
            with autocast(device, enabled=use_amp):
                ...
                # Combined loss with M3 Mac optimized weights
                total_loss = ce_loss + 0.4 * focal_loss + args.proto_weight * proto_loss
                
                # Data augmentation (reduced frequency for stability)
                if args.augment and torch.rand(1) < 0.2:
                    # Speed perturbation
        ...
        if avg_val_f1 > best_f1:
            ...
            torch.save(checkpoint, os.path.join(args.save_dir, f'best_crema_m3_f1_{avg_val_f1:.4f}.pt'))
