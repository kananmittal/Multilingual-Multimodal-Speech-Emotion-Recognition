#!/usr/bin/env python3
"""
Improved CREMA Training Script - Fixed Learning Rate and Configuration Issues
- Increased learning rates for better learning
- Fixed validation loss calculation
- Improved batch size and training stability
...
from data.preprocess import speed_perturb
...
class CosineAnnealingWarmupRestarts(optim.lr_scheduler._LRScheduler):
    """Cosine annealing with warmup and restarts - optimized for better learning"""
...
def main():
    parser = argparse.ArgumentParser(description="Improved CREMA Training")
    ...
    parser.add_argument('--batch_size', type=int, default=32)  # Increased from 20
    # Much higher base LR for actual learning
    parser.add_argument('--lr', type=float, default=2e-4)  # Increased from 1e-4
    parser.add_argument('--warmup_epochs', type=int, default=2)  # Reduced from 3
    parser.add_argument('--use_amp', action='store_true', default=False)
    parser.add_argument('--augment', action='store_true', default=True)  # Enable by default
    parser.add_argument('--proto_weight', type=float, default=0.05)  # Increased from 0.01
    parser.add_argument('--save_dir', type=str, default='checkpoints_crema_improved')
    ...
    parser.add_argument('--patience', type=int, default=8, help='Early stopping patience')  # Increased
    parser.add_argument('--save_every', type=int, default=2, help='Save checkpoint every N epochs')
    parser.add_argument('--seed', type=int, default=42)
    # Prototype loss ramp
    parser.add_argument('--proto_weight_start', type=float, default=0.01)
    parser.add_argument('--proto_weight_end', type=float, default=0.05)
    parser.add_argument('--proto_ramp_epoch', type=int, default=5)
    ...
    print(f"📊 Training on CREMA dataset with improved configuration")
    ...
    # Optimizer with higher learning rates for better learning
    print("⚙️  Setting up optimizer...")
    base_lr = args.lr
    optimizer = optim.AdamW([
        # Encoders: keep trainable but gentle
        {'params': audio_encoder.parameters(), 'lr': base_lr * 0.15, 'weight_decay': 0.01},
        {'params': text_encoder.parameters(), 'lr': base_lr * 0.15, 'weight_decay': 0.01},

        # Attention and pooling
        {'params': cross.parameters(), 'lr': base_lr * 0.6, 'weight_decay': 0.01},
        {'params': pool_a.parameters(), 'lr': base_lr * 0.6, 'weight_decay': 0.01},
        {'params': pool_t.parameters(), 'lr': base_lr * 0.6, 'weight_decay': 0.01},

        # Fusion
        {'params': fusion.parameters(), 'lr': base_lr * 1.0, 'weight_decay': 0.01},

        # Classifier heads
        {'params': classifier.deep_classifier.parameters(), 'lr': base_lr * 1.5, 'weight_decay': 0.01},
        {'params': classifier.anchor_clustering.parameters(), 'lr': base_lr * 0.8, 'weight_decay': 0.01},
        {'params': classifier.uncertainty_head.parameters(), 'lr': base_lr * 0.8, 'weight_decay': 0.01},

        # Prototypes
        {'params': prototypes.parameters(), 'lr': base_lr * 0.6, 'weight_decay': 0.01},
    ...
    if use_amp:
        ...
        print("⚠️  Mixed Precision (AMP) disabled")
    ...
    # Scheduler with better configuration
    scheduler = CosineAnnealingWarmupRestarts(
        optimizer,
        T_0=len(train_loader) * 3,  # Restart every 3 epochs
        T_mult=1,
        eta_min=base_lr * 0.05,  # Higher minimum LR
    ...
    print(f"  Proto ramp: start={args.proto_weight_start}, end={args.proto_weight_end}, epoch={args.proto_ramp_epoch}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Ramp prototype weight
        current_proto_weight = args.proto_weight_start if epoch < args.proto_ramp_epoch else args.proto_weight_end
        ...
        for batch_idx, (audio_list, text_list, labels) in enumerate(pbar):
            ...
            with autocast(device_type=device if device != 'cpu' else 'cpu', enabled=use_amp):
                ...
                if args.augment and torch.rand(1).item() < 0.3:
                    ...
                    total_loss = total_loss + 0.3 * aug_loss
            ...
            with torch.no_grad():
                ...
                train_f1 += f1
                num_batches += 1
                ce_sum += float(ce_loss.detach().cpu())
                focal_sum += float(focal_loss.detach().cpu())
                proto_sum += float(proto_loss.detach().cpu())

            pbar.set_postfix({
                'Loss': f'{(train_loss/num_batches):.4f}',
                'F1': f'{(train_f1/num_batches):.4f}',
                'LR': f'{optimizer.param_groups[0]["lr"]:.2e}',
                'CE': f'{(ce_sum/num_batches):.3f}',
                'Focal': f'{(focal_sum/num_batches):.3f}',
                'Proto': f'{(proto_sum/num_batches):.3f}'
            })
        ...
        with torch.no_grad():
            for audio_list, text_list, labels in tqdm(val_loader, desc="Validation"):
                ...
                with autocast(device_type=device if device != 'cpu' else 'cpu', enabled=use_amp):
                    ...
                    # Validation loss uses same current proto weight
                    total_val_loss = ce_loss + 0.2 * focal_loss + current_proto_weight * proto_loss

                preds = torch.argmax(logits, dim=1)
                f1 = weighted_f1(preds, labels)

                # FIXED: Always add to validation loss
                if torch.isfinite(total_val_loss):
                ...
                val_f1 += f1
                val_batches += 1
                val_ce_sum += float(ce_loss.detach().cpu())
                val_focal_sum += float(focal_loss.detach().cpu())
                val_proto_sum += float(proto_loss.detach().cpu())
        ...
        print(f"  Train CE/Focal/Proto: {(ce_sum/max(1,num_batches)):.4f} / {(focal_sum/max(1,num_batches)):.4f} / {(proto_sum/max(1,num_batches)):.4f}")
        print(f"  Val   CE/Focal/Proto: {(val_ce_sum/max(1,val_batches)):.4f} / {(val_focal_sum/max(1,val_batches)):.4f} / {(val_proto_sum/max(1,val_batches)):.4f}")
        ...
        if avg_val_f1 > best_f1:
            ...
            path = os.path.join(args.save_dir, f'best_crema_improved_f1_{avg_val_f1:.4f}.pt')
