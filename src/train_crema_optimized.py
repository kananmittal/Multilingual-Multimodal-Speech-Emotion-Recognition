#!/usr/bin/env python3
"""
Optimized training script for CREMA dataset with enhanced optimizers and regularization
"""
...
def main():
    parser = argparse.ArgumentParser(description="Optimized CREMA Training")
    ...
    parser.add_argument('--batch_size', type=int, default=8)  # Increased batch size
    parser.add_argument('--lr', type=float, default=2e-4)  # Higher learning rate
    parser.add_argument('--warmup_epochs', type=int, default=2)
    parser.add_argument('--use_amp', action='store_true', default=True)
    parser.add_argument('--augment', action='store_true', default=True)
    parser.add_argument('--proto_weight', type=float, default=0.1)
    parser.add_argument('--save_dir', type=str, default='checkpoints_crema')
    ...
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, collate_fn=collate_fn, num_workers=num_workers, pin_memory=(device != 'mps'))
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, collate_fn=collate_fn, num_workers=num_workers, pin_memory=(device != 'mps'))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, (audio_list, text_list, labels) in enumerate(pbar):
            ...
            with autocast('cuda' if device == 'cuda' else 'cpu', enabled=use_amp):
                # Forward pass
                ...
                # Prototype loss
                proto_loss = prototypes(fused, labels)
